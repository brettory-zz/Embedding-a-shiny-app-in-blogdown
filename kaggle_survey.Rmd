...
For this demonstration I'm going to use data from the freely available [Kaggle survey](https://www.kaggle.com/kaggle/kaggle-survey-2017) on people working in tech. My main research question for today is descriptive: what is the breakdown by gender and country of tech workers?
...
To embed a shiny app on a static site you need to have it published elsewhere online. This can be done for free on [shinyapps.io](https://www.shinyapps.io/). Once you create an account you just follow three very simple instructions on the website to deploy your app. Once deployed, you end up with a url, which you can then embed in an Rmarkdown file using one line of html.  



I deployed an .Rmd file at (https://brettory.shinyapps.io/gender_tech_country/). To get R to recognize an .Rmd file as a shiny app, you need to include the line in the header `runtime: shiny`
...
Because my data manipulations until now weren't very complicated, I just included the above code in my app.Rmd file with echo=FALSE heading each chunk so it would run but wouldn't produce any output. `echo=FALSE` is just one of the many useful commands that can be found on this [Rmarkdown cheat sheet](https://www.rstudio.com/wp-content/uploads/2016/03/rmarkdown-cheatsheet-2.0.pdf).

The resulting file can be found on GitHub, link at the end of this post. 
...
Once created and deployed, I simply embed it in my Rmarkdown file with a little html `<iframe src="myurl"> </iframe>` 
...
```
<iframe width="700" height="700" scrolling="no" frameborder="no"  src="https://brettory.shinyapps.io/gender_tech_country/"> </iframe>
```


Like this:

<iframe width="700" height="400" scrolling="no" frameborder="no"  src="https://brettory.shinyapps.io/gender_tech_country/"> </iframe> It does take a while to load, and I'm not sure why, but I will leave that problem for another day! 
...
It looks like Ireland, Egypt, and Malaysia have the greatest number of women in tech while Japan and Denmark have the least. Ireland, Egypt, and Malaysia also have relatively few tech workers, though, so we may be capturing a bit of random variation. Shockingly, Denmark only has 4 female tech workers who participated in Kaggle's survey of a total of 76, and Japan only has 20 of 274. 


However, that gross gender inequality may not be so bad, as I'll show next time. In other analyses I've conducted, it appears women report the best job satisfaction in countries where there are the least number of women in the tech sector. 
